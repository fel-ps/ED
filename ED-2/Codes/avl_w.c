...
int main() {
    ...
    for (int size = startSize; size <= endSize; size += increment) {
        ...
        // Preencher o array apenas com números pares
        for (int i = 0; i < size; i++) {
            array[i] = (i + 1) * 2;  // Números pares
        }
        ...
        for (int i = 0; i < numExecutions; i++) {
            ...
            // Gerar chaves de busca ímpares que não estão na árvore
            ...
            for (int k = 0; k < numSearches; k++) {
                searchvs[k] = (size + k + 1) * 2 - 1;  // Números ímpares
            }
    ...
    printf("Tempos de execução calculados e salvos em avl_w_time.txt.\n");
